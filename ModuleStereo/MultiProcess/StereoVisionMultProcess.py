...
def coords_mouse_disp(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDBLCLK:
        average=0
        mask = 100
        for u in range (-int(mask//2),int(mask//2)+1):
            for v in range (-int(mask//2),int(mask//2)+1):
                average += disp[y+u,x+v]
        average=average/(mask**2)
        #Distance= 11714*average**(-0.675) 
        #Distance= -593.97*average**(3) + 1506.8*average**(2) - 1373.1*average + 522.06
        Distance= np.around(0.01*70275*average**(-0.976),decimals=2)
...
def TakeStereo(id, t0stereo, frame):
    ...
    # t0stereo = t1stereo
    #Put result
    queue.put((disparity_buffer,grayL,disp))
