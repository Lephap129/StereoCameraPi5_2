...
def capture_frames():
    ...
    while True:
        ...
        elif fps_limit < 80 and psutil.cpu_percent() < 50:
            fps_limit += 10  # Increase the frame rate if CPU is underused
        # if frame_bufferL is None:
        # Start Reading Camera images
...
def TakeStereo():
    ...
    global disparity_buffer, prev_frame_time, t0stereo, fps_stereo, fps_stereo_buffer, frame_buffer, frame_bufferL, fps_cam, fps_cam_buffer, frame
    fps_limit = 100
    while True:
        with buffer_lock:
...
# Start frame stereo thread
stereo_thread = threading.Thread(target=TakeStereo)
stereo_thread.start()
